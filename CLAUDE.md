...
Two sub-agents and one v0 sync layer make up the core:
...
- A fleet manager — the `se` CLI and `stellar-orchestrator.md` agent are planned, not built.
- A Generator agent host — knowledge-source → spec markdown is also planned.
